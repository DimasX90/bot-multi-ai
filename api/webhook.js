...
export default async function handler(request, context) {
  ...
  try {
    ...
    // JIKA MENGETIK /START, BALAS LANGSUNG INSTAN (FIXED SYNTAX STRING ERROR)
    ...
    // 🔥 JALUR CEPAT DETEKSI SALURAN (ANTI-NYANGKUT & BERSIH DUPLIKASI)
...
async function prosesLatarBelakang(chatId, aiPilihan, pesanUser, pesanLowercase, fotoMasuk, dokumenMasuk) {
  try {
    ...
    // 🔥 JALUR DOWNLOAD DIPERBAIKI UNTUK MENDUKUNG GAMBAR & PDF SECARA LENGKAP
    if ((isImage || isPdf) && fileIdToDownload) {
      ...
      if (resFile.ok) {
        ...
        // 🔥 FIX TYPO OPERATOR || DAN FIX EKSTRAKSI DATA BASE64 UNTUK FILE PDF
    ...
    // [4] MODE GROQ CONVERSATIONAL
      ...
      const jawabanGroq = groqData.choices?.[0]?.message?.content || "⚠️ Gagal.";
    ...
    // [5] MODE SUPER KILAT VIA LLAMA 4 SCOUT (GROQ)
      ...
      try {
        ...
        } else { await kirimPesanTelegram(chatId, `⚠️ Error API Groq`); }
...
Format Rumus: Wajib bungkus rumus pendek/inline dengan $...$ dan rumus panjang/matriks/display dengan $$...$$.`;
      ...
      let isiKonten = [{ text: `${instruksiPakar}\n\nPerintah Tambahan: ${pertanyaanClean || "Selesaikan seluruh soal pada file referensi ini secara urut menggunakan rumus resmi."}` }];
